...
import database.create.CreatePracticeContest;
import database.delete.DeleteContest;
import database.delete.DeleteOnlineContest;
import database.delete.DeleteTestContest;

@SuppressWarnings("serial")
class PopUpMenuTestOnline extends JPopupMenu implements ActionListener {
	JMenuItem Item1, Item2, Item3, Item4;
	String contestname;
	GridLayoutContests cp;

	public PopUpMenuTestOnline(String contestname, GridLayoutContests cp) {
		this.cp = cp;
		Item1 = new JMenuItem("Put Offline", new ImageIcon(
				PopUpMenuTestOnline.class.getResource("/img/putoffline.png")));
		Item1.setBorder(new EmptyBorder(5, 5, 5, 5));
		Item1.setFont(new Font("SanSerif", Font.BOLD, 12));

		Item4 = new JMenuItem("Enable Practice Mode", new ImageIcon(
				PopUpMenuTestOnline.class
						.getResource("/img/practicetype_small.png")));
		...
		Item2 = new JMenuItem("Delete Contest", new ImageIcon(
				PopUpMenuTestOnline.class
						.getResource("/img/trashcan_delete2.png")));
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if (event.equals("Put Offline")) {
			...
			PopUpMenuTestOnline.this.register(cp);
		...
		} else if (event.equals("Enable Practice Mode")) {
			new CreatePracticeContest(contestname);
			new DeleteTestContest(contestname);
			PopUpMenuTestOnline.this.register(cp);
		}
	}
